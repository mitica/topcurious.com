...
markdown.renderer.rules.image = function (tokens, idx, options, env, self) {
  ...
  const img = `<img loading="lazy" srcset="${smallSrc} 400w, ${imgSrc} 800w" sizes="(max-width: 560px) 400px, 800px" alt="${imgAlt}"/>`;
...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addNunjucksShortcode("youtubeLightIcon", function () {
    return `<svg width="24" height="24" aria-hidden="true" role="img" width="1em" height="1em" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
      viewBox="0 0 461.001 461.001" xml:space="preserve">
    <g>
      <path fill="#ffffff" d="M365.257,67.393H95.744C42.866,67.393,0,110.259,0,163.137v134.728
  ...
  eleventyConfig.addFilter("localeUrl", function (url, locale) {
    return locale ? `/${locale}${url}` : url;
  });

  eleventyConfig.addFilter("tagParam", simpleTag);
  eleventyConfig.addFilter("langNativeName", getLangNativeName);
  ...
  // eleventyConfig.addCollection("locale-tags-", function (collectionApi) {
  //   return collectionApi.get().sort(function (a, b) {
  //     return b.date - a.date;
  //   });
  // });

  languages.forEach((language) =>
    eleventyConfig.addCollection(`posts_${language}`, function (collection) {
      return collection.getFilteredByGlob(`./content/${language}/**/*.md`);
    })
  );

  languages.forEach((language) =>
    eleventyConfig.addCollection(
      `locale_tags_${language}`,
      function (collection) {
  ...
  eleventyConfig.addFilter("postsByTag", function (collection, tag, limit) {
    ...
    for (const item of collection) {
      ...
      if (uniqueposts.size >= limit) break;
    }
    return [...uniqueposts];
  });
